...
class TFBExecutor(object):
  """A base model that will use interact with our database 
  scripts. 

  Executor lifecycle: 
   - setup will be called each time the source repository or 
     commit to be tested changes
   - execute will be called in between calls to setup, and 
     should run benchmarks and return results
   - stop is called before the executor is deallocated. All 
     resources should be cleaned up here
  """
  
  def __init__(self, commit, logger=None):
    """Store critical variables, such as connection info"""
  ...
  def setup(self):
    """Do any long-running operations needed to setup the 
    executor, such as allocating and configuring servers, 
    generating images/snapshots, etc. Because TFB's setup
    can change drastically from commit to commit

    Called before execute"""
    pass

  def execute(self):
    """Benchmark the framework tests as they existed in 
    the given repository at the given commit. Will be called
    many times, needs to reset executor state. Should return
    benchmark results or throw exception"""
    pass

  def stop(self):
    """Do any shutdown tasks required. Guarantee is that this
    will be called at least once before shutdown"""
...
class DockerExecutor(TFBExecutor):
  ...
  def start(self):
    ...
    self.build_container(os.path.dirname(os.path.realpath(__file__)) + '/docker/ssh', self.base_image)

    # Sets up fabric SSH library
    env.key_filename = os.path.dirname(os.path.realpath(__file__)) + '/docker/ssh/id_rsa'   # Use private key file
  ...
  def execute(self, framework_tests=[]):
    execute(self._execute, hosts=[self.server_fab], framework_tests=framework_tests)

  def _execute(self, framework_tests):
    frameworks = " ".join(framework_tests)
    with cd('~/FrameworkBenchmarks'):
      command = 'python toolset/run-tests.py --install server --verbose --runner-user tfbrunner '
